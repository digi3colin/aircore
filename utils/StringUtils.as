package utils {
	...
	final public class StringUtils {
		...
		public static function entityDecode(str:String):String{			
			...
			var matches:Array =str.match(/&([a-zA-Z]+)(;?)/g).sort();
			var lastMatch:String;
			var reg:RegExp;
			for(var i:int=0; i<matches.length;i++){
				if(lastMatch == matches[i])continue;
				reg = new RegExp(matches[i],'g');
		...
		static public function capitalizeWord(str:String):String{
			var wordArray:Array = str.split(" ");
			for (var i:Number = 0; i<wordArray.length; i++) {
			    wordArray[i] = 
				String(wordArray[i]).substr(0, 1).toUpperCase()+
				String(wordArray[i]).substr(1).toLowerCase();
			}
